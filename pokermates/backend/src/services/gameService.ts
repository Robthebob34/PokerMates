...
import type { PlayerHand, Game, RoomPlayer } from '@prisma/client';

const prisma = new PrismaClient();

type ExtendedPlayerHand = PlayerHand & {
  player: RoomPlayer & { chips: number };
};
...
export const gameService = {
  ...
  async playerAction(handId: string, playerId: string, action: 'fold' | 'check' | 'call' | 'raise', amount?: number) {
    ...
    return await prisma.$transaction(async (tx: Prisma.TransactionClient) => {
      ...
      const playerHand = hand.playerHands.find((ph: { playerId: string }) => ph.playerId === playerId);
      if (!playerHand) {
      ...
      // Define LocalPlayerHand interface for type safety in this function
      interface LocalPlayerHand {
        id: string;
        playerId: string;
        isFolded: boolean;
        isAllIn: boolean;
        betAmount: number;
        cards: string; // This is a string in the database
        isHost?: boolean;
      }

      // Determine the next player with proper typing
      const activePlayers = hand.playerHands.filter((ph: LocalPlayerHand) => !ph.isFolded && !ph.isAllIn);
      const currentPlayerIndex = activePlayers.findIndex((ph: LocalPlayerHand) => ph.playerId === playerId);
      ...
      const allPlayersActed = activePlayers.every((ph: LocalPlayerHand) => 
        ph.betAmount === hand.game.currentBet || ph.isAllIn
      );
      ...
      if (allPlayersActed) {
        let nextStatus: 'FLOP' | 'TURN' | 'RIVER' | 'SHOWDOWN' | 'COMPLETED';
