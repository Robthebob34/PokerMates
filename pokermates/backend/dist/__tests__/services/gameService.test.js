...
const mockExpect = (value) => ({
    toBe: (expected) => console.assert(value === expected, `Expected ${expected}, got ${value}`),
    toBeDefined: () => console.assert(value !== undefined, 'Expected value to be defined'),
    toHaveLength: (length) => console.assert(Array.isArray(value) ? value.length === length : value?.length === length, `Expected length ${length}, got ${Array.isArray(value) ? value.length : value?.length}`)
});
...
async function runTests() {
    ...
    try {
        ...
        const result = await gameService_js_1.gameService.startGame(testRoomId);
        ...
        // Verify player hands
        console.assert(result.hand.playerHands.length === 4, `Expected 4 player hands, got ${result.hand.playerHands.length}`);
        // Verify each player's hand
        result.hand.playerHands.forEach((hand) => {
        ...
        try {
            await gameService_js_1.gameService.startGame(testRoomId);
            console.error('Error: Expected startGame to throw an error but it did not');
        }
        catch (error) {
            console.assert(error.message.includes('already in progress'), `Expected 'already in progress' error, got: ${error.message}`);
